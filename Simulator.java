	...
	public void simulate() {
		...
		while (clock < steps) {
			...
			if (RandomGenerator.eventOccurred(probabilityOfArrivalPerSec) == true) {
				temp = new Spot(RandomGenerator.generateRandomCar(), clock);
			...
			if (incomingQueue.isEmpty() != true) {
				if (dequeued == null) {
					dequeued = incomingQueue.dequeue();
					if (lot.attemptParking(dequeued.getCar(), clock) == true) {
						lot.attemptParking(dequeued.getCar(), clock);

						System.out.println(dequeued.getCar().toString() + " ENTERED at timestep " + clock + "; occupany is at " + lot.getTotalOccupancy());
						dequeued = null;
					}
				} else if (lot.attemptParking(dequeued.getCar(), clock) == true) {
						lot.attemptParking(dequeued.getCar(), clock);
			...
			if (outgoingQueue.isEmpty() != true) {
				leaving = outgoingQueue.dequeue();
				leaving.setTimestamp(clock);

				System.out.println(leaving.getCar().toString() + " EXITED at timestep " + clock + "; occupany is at " + lot.getTotalOccupancy());
				leaving = null;
			}
