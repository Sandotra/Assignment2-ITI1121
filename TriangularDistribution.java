...
public class TriangularDistribution {
	...
	public Rational pdf(int x) {
		...
		// WRITE YOUR CODE HERE!
		Rational pdf = new Rational(0, 1);
		if (x < a || b < x) {
			return pdf;
		} else if (a <= x && x < c) {
			pdf = new Rational((2*(x-a)), ((b-a)*(c-a)));
		} else if (x == c) {
			pdf = new Rational(2, b-a);
		} else if (c < x && x <= b) {
			pdf = new Rational((2*(b-x)), ((b-a)*(b-c)));
		}

		return pdf; // Remove this statement when your implementation is complete.
	}
